...
const filteredItems = computed(() => {
  return context.items.filter(
    item => item.key == null || app.features.includes(item.key)
  );
});
...
app.emitter.on("vf-contextmenu-show", ({ event, items, target = null }) => {
  ...
  if (searchQuery.value) {
    ...
    if (target.type === "dir") {
      ...
      if (
        app.pinnedFolders.findIndex(item => item.path === target.path) !== -1
      ) {
    ...
    if ((target.mime_type || target.mimeType) === "application/zip") {
...
const showContextMenu = event => {
  ...
  nextTick(() => {
    ...
    left =
      areaContainer.right - event.pageX + window.scrollX < menuWidth
        ? left - menuWidth
        : left;
    top =
      areaContainer.bottom - event.pageY + window.scrollY < menuHeight
        ? top - menuHeight
        : top;
