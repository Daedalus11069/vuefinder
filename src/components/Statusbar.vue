  ...
  <div class="vuefinder__status-bar__wrapper">
    <div class="vuefinder__status-bar__storage">
      ...
      <div class="vuefinder__status-bar__info">
        <span v-if="searchQuery.length"
          >{{ app.fs.data.files.length }} items found.
        </span>
        <span class="vuefinder__status-bar__selected-count">{{
          app.dragSelect.getCount() > 0
            ? t("%s item(s) selected.", app.dragSelect.getCount())
            : ""
        }}</span>
      </div>
    </div>
    <div class="vuefinder__status-bar__actions">
      <button
        ...
        :class="{ disabled: !isAdditionalButtonActive(btn) }"
        :disabled="!isAdditionalButtonActive(btn)"
        @click="btn.click(ds.getSelected(), $event)"
        v-for="btn in app.additionalButtons"
      >
        {{ btn.label }}
      </button>
      <button
